...
// #region Configuration & Dictionaries
...
const LANG_NAME_MAP: { [key: string]: string } = {
  한국: "한국어",
  캄보디아: "캄보디아어",
  네팔: "네팔어",
  필리핀: "필리핀어",
...
// #region Helper Functions (Logic)
function normalizeKey(str: any): string {
  if (!str) return "";
  return String(str).trim().replace(/\s+/g, " ");
}

function generateTransCode(text: string): string {
  if (!text) return "";
...
function UploadPage({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = processExcelFile(arrayBuffer, file.name);
...
// #region Business Logic (Browser Compatible)
function processExcelFile(arrayBuffer: ArrayBuffer, fileName: string) {
